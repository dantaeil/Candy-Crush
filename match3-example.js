...
window.onload = function () {
  ...
  // Special Tile colors in RGB
  var specialtilecolors = [
    [0, 0, 0],
    [255, 0, 0],
    [0, 255, 0],
  ];
    ...
    for (var i = 0; i < level.columns; i++) {
      for (var j = 0; j < level.rows; j++) {
        ...
        if (level.tiles[i][j].type >= 0) {
          let col;
          // Render the special tile
          switch (level.tiles[i][j].type) {
            case CANDY:
              col = specialtilecolors[0];
              break;
            case SWEET:
              break;
            case BOMB:
            ...
            default:
              ...
              col = tilecolors[level.tiles[i][j].type];
          }
  ...
  function loopClusters() {
    for (var i = 0; i < clusters.length; i++) {
      ...
      var coffset = 0;
      var roffset = 0;
      if (cluster.length > 3) console.log("Candy has been detected");
      for (var j = 0; j < cluster.length; j++) {
        if (cluster.length == 4 && j == 0)
          level.tiles[cluster.column + coffset][cluster.row + roffset].type =
            CANDY;
        else
          level.tiles[cluster.column + coffset][cluster.row + roffset].type =
            -1;
